...
public class Arrival implements Comparable<Arrival>{
    ...
    public String getTravelDirn() {
        int l = platform.indexOf("-");
        if (l > 0) {
            return platform.substring(0, l).trim();
        }
        return "";
    }
    ...
    public String getPlatformName() {
        int l = platform.indexOf("-");
        if ( l > 0) {
            return platform.substring(l, platform.length() - 1).trim();
        }
        return "";
    }
    ...
    public int getTimeToStationInMins() {
        int seconds = 60;
        if(timeToStation % seconds == 0) {
            return timeToStation / seconds;
        }
        else {
            return timeToStation / seconds + 1;
        }
    }
